                       ...
                       toolName:(NSString *)toolName
{
    ...
    if ([executable isEqualToString:@"/usr/bin/env"] &&
        ...
        [runtimeArguments.firstObject isEqualToString:@"node"]) {
        [runtimeArguments removeObjectAtIndex:0];
    }

    NSString *sourceKind = @"none";
    NSString *source = nil;
    if (runtimeArguments.count >= 2 && [runtimeArguments.firstObject isEqualToString:@"-e"]) {
        sourceKind = @"inline";
    ...
    NSMutableDictionary *request = [NSMutableDictionary dictionaryWithDictionary:@{
        ...
        @"conversationID": sid ?: @"",
        @"toolID": toolName ?: @"NodeJSRuntimeTool",
        @"policyProfileID": @"runtime.node.v1",
        @"runtime": @"node",
        ...
        @"environment": environment ?: @{},
        @"timeoutSeconds": @(timeout),
        @"outputLimitBytes": @(NodeJSRuntimeMaxOutputBytes)
    }];
    if (source) {
