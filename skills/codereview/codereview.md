Review the source-code root directory '{{directory}}' none recursive file by file.

Create a target subdirectory inside {{review_root_dir}} by preserving the source tree structure and write one Markdown review file for each reviewed source file using this output path pattern:
...
After all per-file reviews are written, create {{review_root_dir}}/<relative/source/path>/README.md with total files reviewed, counts by priority, top correction themes, recommended fix order, and verification commands.