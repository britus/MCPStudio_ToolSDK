...
def test_policy_filters_objective_excluded_sections_before_chunking(
    tmp_path: Path,
) -> None:
    doc_dir = tmp_path / "runtime-sources"
    doc_dir.mkdir()
    (doc_dir / "mixed.txt").write_text(
        "TARGET INTRO\n"
        + "".join(f"target line {index}\n" for index in range(12))
        + "OUTSIDE SECTION\n"
        + "".join(f"excluded fact {index}\n" for index in range(12))
        + "TARGET RESUMES\n"
        + "".join(f"more target line {index}\n" for index in range(12)),
        encoding="utf-8",
    )
    ...
    manifest = prepare_documents(
        ...
        policy={
            "format": 2,
            "requiredSubjects": ["target-subject"],
            "contentScope": {
                "targets": ["documented target"],
                "boundaries": ["keep target and non-target material separate"],
                "exclusions": ["facts that apply only outside the target"],
            },
            "sources": [
                {
                    "path": "mixed.txt",
                    ...
                    "contentAssessment": {
                        ...
                        "evidence": "The source has an explicitly headed outside section.",
                    },
                    "contentRules": [
                        {
                            "id": "exclude-outside-section",
                            "action": "exclude-section",
                            "start": "OUTSIDE SECTION",
                            "end": "TARGET RESUMES",
                            "origin": "machine-derived",
                            "reason": "The objective excludes facts that apply only outside the target.",
                            "evidence": "The two unique headings bound the outside section.",
                        }
                    ],
                }
            ],
    ...
    completions = "\n".join(
        message["content"]
        for record in records
        for message in record["completion"]
    )
    report = manifest["dataset_policy"]
    assert "excluded fact" not in completions
    assert "target line" in completions
    assert "more target line" in completions
    assert report["content_scope"]["targets"] == ["documented target"]
    assert report["content_filtering_enabled"] is True
    assert report["excluded_line_count"] == 13
    assert report["content_filter_rules_applied"][0]["id"] == "exclude-outside-section"
    assert report["content_assessments"][0]["classification"] == "mixed-filtered"


def test_policy_rejects_mixed_source_without_content_rules(tmp_path: Path) -> None:
    doc_dir = tmp_path / "runtime-sources"
    doc_dir.mkdir()
    (doc_dir / "mixed.txt").write_text("target and unrelated content\n")

    with pytest.raises(ValueError, match="mixed-filtered but has no contentRules"):
        prepare_documents(
            [doc_dir],
            tmp_path / "train.jsonl",
            tmp_path / "validation.jsonl",
                ...
                "sources": [
                    {
                        ...
                        "contentAssessment": {
                            "classification": "mixed-filtered",
                            "evidence": "The source mixes target and unrelated content.",
                        },
                        "contentRules": [],
                    }
                ],
...
def test_policy_rejects_unresolved_content_rule_anchor(tmp_path: Path) -> None:
    doc_dir = tmp_path / "runtime-sources"
    doc_dir.mkdir()
    (doc_dir / "mixed.txt").write_text("TARGET\nOUTSIDE\nTARGET AGAIN\n")
                ...
                "sources": [
                    {
                        ...
                        "contentAssessment": {
                            "classification": "mixed-filtered",
                            "evidence": "The source includes an outside section.",
                        },
                        "contentRules": [
                            {
                                "id": "bad-anchor",
                                ...
                                "end": "TARGET AGAIN",
                                "origin": "machine-derived",
                                "reason": "Exclude the outside section.",
                                "evidence": "The objective excludes outside material.",
                            }
                        ],
                    }
                ],
...
def test_policy_rejects_ambiguous_content_rule_anchor(tmp_path: Path) -> None:
    doc_dir = tmp_path / "runtime-sources"
    doc_dir.mkdir()
    (doc_dir / "mixed.txt").write_text(
        "TARGET\nOUTSIDE\nTARGET AGAIN\nOUTSIDE\nTARGET END\n",
    ...
    with pytest.raises(ValueError, match="anchor is ambiguous"):
        prepare_documents(
            [doc_dir],
            tmp_path / "train.jsonl",
            tmp_path / "validation.jsonl",
                ...
                "sources": [
                    {
                        ...
                        "contentAssessment": {
                            "classification": "mixed-filtered",
                            "evidence": "The source includes repeated outside sections.",
                        },
                        "contentRules": [
                            {
                                "id": "ambiguous-anchor",
                                ...
                                "end": "TARGET END",
                                "origin": "machine-derived",
                                "reason": "Exclude outside content.",
                                "evidence": "The objective excludes outside material.",
                            }
                        ],
                    }
